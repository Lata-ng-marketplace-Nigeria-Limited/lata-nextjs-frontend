...
interface Props {
  ...
  title?: string;
  singleSellerMode?: boolean;
}

export const ReelsRow = ({ reelsGrouped, title, singleSellerMode }: Props) => {
  ...
  const isSingleSeller = singleSellerMode ?? activeGroups.length === 1;

  return (
    <div className="w-full mt-6 sl:mt-8">
      {title && (
        <h2 className="text-xs font-semibold text-grey9 sm:text-base sm:font-medium mb-3">
          {title}
        </h2>
      )}
            ...
            return (
              <div
                ...
                onClick={() => {
                  ...
                  setSelectedSellerId(activeGroups[0].user_id);
                }}
